...
class ProjectAPI {
  ...
  static mutateHighlights(variables) {
    let body = new FormData();
    body.set("config", JSON.stringify(variables.config));

    const url = api_url + `projects/${variables.project_id}/highlights`;
    return new Promise((resolve, reject) => {
      axios({
        method: "put",
        url: url,
  ...
  static fetchZoteroConfig({ queryKey }) {
    const { project_id } = queryKey[1];
  ...
  static mutateZoteroConfig(variables) {
    let body = new FormData();
    body.set("config", JSON.stringify(variables.config));

    const url = api_url + `projects/${variables.project_id}/zotero`;
    return new Promise((resolve, reject) => {
      axios({
        method: "put",
        url: url,
